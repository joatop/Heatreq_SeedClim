...
comdat <- comdat %>% 
  ...
  mutate(species_name=str_replace(species_name,"Minuartia biflora", "Cherleria biflora")) %>%
...
summary(comind)
dim(comind) # 1380 of 20854 observations are without indicator values for heat requirement
...
# listing all naturally alpine species for TT2
sp.alp <- unique(
  comind %>% 
    filter(alt.orig %in% list("alpine")) %>%
    filter( (year %in% list(2009) & TTtreat %in% list("TT2"))) %>%
    select(species_name)
)[,1]
